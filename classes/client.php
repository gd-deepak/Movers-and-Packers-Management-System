...
Class client extends DBConnection {
	...
	public function save_client(){
		...
		if(empty($id)){
			$qry = $this->conn->query("INSERT INTO client1 set {$data}");
			if($qry){
				...
				if(!empty($_FILES['img']['tmp_name'])){
					...
					if($upload){
						$this->conn->query("UPDATE `client1` set `avatar` = CONCAT('{$fname}', '?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$id}'");
						if($this->settings->userdata('id') == $id)
			...
			$qry = $this->conn->query("UPDATE client1 set $data where id = {$id}");
			if($qry){
				...
				return "UPDATE client1 set $data where id = {$id}";
			}
	...
	public function delete_client(){
		extract($_POST);
		$qry = $this->conn->query("DELETE FROM client1 where id = $id");
		if($qry){
